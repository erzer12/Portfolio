...
function SortableCertificationItem({
	...
	return (
		<div
			ref={setNodeRef}
			style={style}
			className="flex items-center justify-between gap-4 border-b border-[--rule] py-2 bg-[--bg]"
		>
			...
			<div className="flex gap-2">
				{/* Fixed: Added explicit type="button" */}
				<button type="button" onClick={() => onEdit(cert)} className="admin-btn-sm">
					Edit
				</button>
				{/* Fixed: Added explicit type="button" */}
				<button
			...
			{editing && (
				<form onSubmit={handleSave} className="space-y-4 border-t border-[--rule] pt-6">
					{[
						...
						<div key={name}>
							{/* Fixed: Added htmlFor attribute mapping */}
							<label
								htmlFor={name}
							...
							{/* Fixed: Added matching id attribute */}
							<input
								id={name}
								name={name}
								value={(editing[name as keyof Certification] as string) ?? ''}
