  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        {education.map((edu) => (
          <div key={edu.id} className="flex items-start justify-between gap-4 border-b border-[#E4E4DF] py-2">
            <div>
              <p className="text-[14px] font-medium text-[#1A1A18]">{edu.school}</p>
              <p className="font-mono text-xs text-[#6B6B66]">{edu.degree} · {edu.year}</p>
            </div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="space-y-4 border-t border-[#E4E4DF] pt-6">
          ...
          <div>
            <label className="mb-1 block font-mono text-xs uppercase tracking-[0.12em] text-[#6B6B66]">Notes (optional)</label>
            <textarea name="description" value={editing.description ?? ''} onChange={handleChange} rows={2} className="admin-input" />
          </div>
          <div>
            <label className="mb-1 block font-mono text-xs uppercase tracking-[0.12em] text-[#6B6B66]">Sort Order</label>
