	...
	return (
		...
		>
			...
			<div className="flex gap-2">
				...
				<button type="button" onClick={() => onEdit(edu)} className="admin-btn-sm">
					Edit
				</button>
				{/* Fixed: Added type="button" */}
				<button
			...
			{editing && (
				<form onSubmit={handleSave} className="space-y-4 border-t border-[--rule] pt-6">
					{[
						...
						<div key={name}>
							{/* Fixed: Added htmlFor structural link */}
							<label
								htmlFor={name}
							...
							{/* Fixed: Added matching id */}
							<input
								id={name}
								name={name}
								value={(editing[name as keyof Education] as string) ?? ''}
