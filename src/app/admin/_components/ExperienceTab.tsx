...
function SortableExperienceItem({
	...
	return (
		<div
			ref={setNodeRef}
			style={style}
			className="flex items-start gap-4 border-b border-[--rule] py-2 bg-[--bg]"
		>
			<div
				{...attributes}
				{...listeners}
				className="cursor-grab text-[--ink-faint] hover:text-[--ink] active:cursor-grabbing px-2 pt-1"
			>
				☰
			</div>
			<div className="flex-1">
				...
				<p className="font-mono text-xs text-[--ink-muted]">
					{ex.role} {ex.employment_type ? `(${ex.employment_type})` : ''} · {ex.start_date}–
					{ex.end_date ?? 'Present'}
				</p>
			</div>
			<div className="flex gap-2">
				<button type="button" onClick={() => onEdit(ex)} className="admin-btn-sm">
					Edit
				</button>
				<button
					type="button"
				...
				<form onSubmit={handleSave} className="space-y-4 border-t border-[--rule] pt-6">
					{[
						...
						<div key={name}>
							...
							<input
								id={name}
								name={name}
								value={(editing[name as keyof Experience] as string) ?? ''}
					...
					<div>
						...
						<p className="mt-1 font-mono text-xs text-[--ink-faint]">
							Use the slug from the Projects tab (e.g. &apos;newshunt&apos;)
						</p>
					</div>
					<div>
						<label
