	...
	return (
		<div ref={setNodeRef} style={style} className="flex items-center gap-3 py-1.5 bg-[--bg]">
			<div
				{...attributes}
				{...listeners}
				className="cursor-grab text-[--ink-faint] hover:text-[--ink] active:cursor-grabbing text-sm"
			>
				☰
			</div>
			...
			<div className="flex gap-1 shrink-0">
				...
				<button type="button" onClick={() => onEdit(link)} className="admin-btn-sm">
					Edit
				</button>
				{/* Fixed: Added type="button" */}
				<button
	...
	return (
		<div className="space-y-8">
				...
				<SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
					{categories.length === 0 && (
						<p className="font-mono text-xs text-[--ink-faint]">
							No footer links yet. Add a category to get started.
						</p>
					)}
					{categories.map((category) => (
						<div key={category} className="space-y-2">
							...
							<div className="flex items-center justify-between border-b border-[--rule] pb-1">
								<h3 className="font-mono text-xs uppercase tracking-[0.14em] text-[--ink]">
									{category}
								</h3>
								{/* Fixed: Added type="button" */}
								<button
								...
								{grouped[category].map((link) =>
									editing && editing.id === link.id ? (
										<form
											key={link.id}
											onSubmit={handleSave}
											className="flex items-center gap-3 py-1.5 bg-[--bg]"
										>
											<div className="flex-1 min-w-0">
			...
			<div className="border-t border-[--rule] pt-4">
				{!showNewCategory ? (
					/* Fixed: Added type="button" */
					<button type="button" onClick={() => setShowNewCategory(true)} className="admin-btn">
						+ New Category
					</button>
				) : (
			...
			{editing && (
				<form onSubmit={handleSave} className="space-y-4 border border-[--rule] p-4">
					...
					{editing.id && (
						<div>
							{/* Fixed: Connected htmlFor with id */}
							<label
