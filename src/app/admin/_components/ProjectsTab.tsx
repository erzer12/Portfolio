...
function SortableProjectItem({
	...
	return (
		<div
			ref={setNodeRef}
			style={style}
			className="flex items-center gap-4 border-b border-[--rule] py-2 bg-[--bg]"
		>
			<div
				{...attributes}
				{...listeners}
				className="cursor-grab text-[--ink-faint] hover:text-[--ink] active:cursor-grabbing px-2"
			>
				☰
			</div>
			...
			<div className="flex gap-2">
				<button type="button" onClick={() => onEdit(p)} className="admin-btn-sm">
					Edit
				</button>
				<button
					type="button"
...
export function ProjectsTab({ projects }: Props) {
	...
	return (
		<div className="space-y-6">
			...
			<button type="button" onClick={handleNew} className="admin-btn">
				+ Add Project
			</button>
				...
				<form onSubmit={handleSave} className="space-y-4 border-t border-[--rule] pt-6">
					<div className="flex items-center justify-between">
						...
						<div className="flex gap-2 items-center">
							<label htmlFor="github-import-url" className="sr-only">
							...
							<button
								...
								className="admin-btn-sm whitespace-nowrap"
							>
					...
					{[
						...
						<AdminField key={name} id={name} label={label}>
							<input
								id={name}
								name={name}
					...
					<AdminField id="image" label="Project Image">
					...
					<AdminField id="description" label="Short Description (main page)">
						<textarea
							id="description"
							name="description"
					...
					<AdminField id="long_description" label="Full Description (detail page)">
						<textarea
							id="long_description"
					...
					<AdminField id="tags" label="Tags (comma separated)">
						<input
							id="tags"
							name="tags"
					...
					<div className="flex items-center gap-2">
						<input
							...
							onChange={handleChange}
						/>
						<label htmlFor="featured" className="font-mono text-xs text-[--ink] cursor-pointer">
