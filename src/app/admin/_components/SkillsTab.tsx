    ...
    <div className="space-y-6">
      <div className="space-y-2">
        {skills.map((s) => (
          ...
          >
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.1em] text-[#6B6B66]">
                {s.category}
              </p>
              <p className="text-[14px] text-[#1A1A18]">{s.skills.join(' · ')}</p>
            </div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="space-y-4 border-t border-[#E4E4DF] pt-6">
          ...
          <div>
            <label className="mb-1 block font-mono text-xs uppercase tracking-[0.12em] text-[#6B6B66]">Skills (comma separated)</label>
            <input name="skills" value={(editing.skills ?? []).join(', ')} onChange={handleChange} className="admin-input" />
          </div>
          <div>
            <label className="mb-1 block font-mono text-xs uppercase tracking-[0.12em] text-[#6B6B66]">Order</label>
