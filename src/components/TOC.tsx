...
export default function TOC({ items }: { items: Item[] }) {
	...
	useEffect(() => {
		...
		return () => {
			obs.disconnect();
			/* Fixed: Wrapped fn() execution in curly braces to drop implicit callback returns */
	...
	return (
		<nav
			...
			className="hidden lg:flex fixed right-6 top-1/2 z-40 -translate-y-1/2 flex-col items-end gap-3"
		>
			{items.map((it) => {
				const isOn = active === it.id || hover === it.id;
				return (
					/* Fixed: Added explicit type="button" layout modifier */
					<button
						type="button"
						key={it.id}
						onClick={() => go(it.id)}
					...
					>
						<span
							className={`block w-3 h-10 rounded-l-full transition-colors ${isOn ? 'bg-[--ink]' : 'bg-[--rule]'}`}
						/>
						<span
							className={
