...
export function MinimalFooter({ links }: Props) {
	...
	return (
		<footer className="mt-16 border-t border-[--rule]">
			...
			<div
				className={`border-t border-[--rule] py-6 flex items-center justify-between gap-4 flex-wrap ${!hasLinks ? 'mt-0' : ''}`}
			>
				<p className="font-mono text-[11px] text-[--ink-faint]">
					© {new Date().getFullYear()} Harshil P
				</p>
				<p className="font-mono text-[11px] text-[--ink-faint]">Built with Next.js & Supabase</p>
			</div>
		</footer>
	);
}
