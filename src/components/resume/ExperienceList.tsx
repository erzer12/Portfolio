				...
				return (
					<article key={item.id} className="space-y-3">
						...
						<p className="text-[14px] italic text-[--ink-muted]">
							...
							{item.employment_type ? ` · ${displayText(item.employment_type)}` : ''}
						</p>
						...
						{item.bullets.length > 0 && (
							<ul className="space-y-2 pl-4 text-[14px] leading-7 text-[--ink]">
								{item.bullets.map((bullet) => (
									<li key={`${item.id}-bullet-${bullet.slice(0, 15).replace(/[^a-z0-9]/gi, '')}`}>
										· {bullet}
									</li>
								))}
							</ul>
							...
							<div className="pt-1 space-y-1">
								...
								<div className="flex flex-wrap gap-2">
									{linkedProjects.map((project) => (
										/* Fixed: Repaired broken template literal tracking */
										<a
											key={project.slug}
