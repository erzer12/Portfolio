...
export async function reseedPortfolioData() {
	...
	await supabaseAdmin.from('skills').insert([
		...
		{
			category: 'Web and Application Dev',
			skills: ['REST APIs', 'Flask', 'React', 'Next.js', 'Gradio', 'Streamlit'],
			order: 2,
		},
		{
		...
		{
			category: 'Databases and Platforms',
			skills: ['MongoDB', 'PostgreSQL', 'Firebase', 'Vercel', 'Render', 'Jupyter'],
			order: 5,
		},
		{
			category: 'Tools and Libraries',
			skills: ['Git', 'MediaPipe', 'ONNX Runtime', 'TanStack Query', 'Zustand', 'Tailwind CSS'],
			order: 6,
		},
	]);
	...
	await supabaseAdmin.from('experience').insert([
		...
		},
		{
			...
			certificate_url: null,
			recommendation_url:
				'https://drive.google.com/file/d/1PJKXCFVBAHIPTnlXYEgLeKA3sdHhOIr3/view?usp=sharing',
			repo_url: null,
			related_projects: [],
			order: 1,
		},
	...
	await supabaseAdmin
		.from('site_settings')
		.upsert({ id: 'main', show_testimonials: true }, { onConflict: 'id' });

	await supabaseAdmin.from('footer_links').insert([
		{
			category: 'Professional',
			label: 'LinkedIn',
			url: 'https://linkedin.com/in/harshilp1',
			order: 0,
		},
		{
			category: 'Professional',
			label: 'Credly',
			url: 'https://www.credly.com/users/harshilp/',
			order: 1,
		},
		{ category: 'Coding', label: 'GitHub', url: 'https://github.com/erzer12', order: 2 },
		{
			category: 'Coding',
			label: 'GeeksforGeeks',
			url: 'https://www.geeksforgeeks.org/profile/harshilp07',
			order: 3,
		},
		{
			category: 'Coding',
			label: 'HackerRank',
			url: 'https://www.hackerrank.com/profile/erzer',
			order: 4,
		},
